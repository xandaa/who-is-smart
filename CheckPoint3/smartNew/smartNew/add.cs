    ...
    public partial class add : Form
    {
        ...
        private void finishBtn_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < cnt; i++)
            {
                ...
                String que = question[i].ToString().Replace("'","''");
                String ans = answer[i].ToString().Replace("'","''");
                string query = "INSERT INTO `smart`.`problems` (`l_id`, `level`, `question`, `answer`) VALUES (" + l_id + ", " + lvl + ", '" + que + "', '" + ans + "')";
                MySqlCommand new_command = new MySqlCommand(query, new_con);
                new_command.ExecuteNonQuery();
            }

            new_con.Close(); new_con.Open();
            String lname = nm.ToString().Replace("'", "''");
            string query1 = "INSERT INTO `smart`.`lessons` (`l_id`, `l_name`, `level`) VALUES (" + l_id + ", '" + lname + "', '" + lvl + "')";
